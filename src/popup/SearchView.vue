...
<v-app :style="'width: '+mainWidth+'px'">
  <v-toolbar app height="70">
    <v-layout align-center>
      <v-flex xs2>
        <img :src="loadExtFile('assets/logo.svg')" height="36" width="36">
      </v-flex>
      <v-flex xs10>
        <v-text-field
          autofocus
      ...
      <v-list two-line>
        ...
        >
          <v-list-tile-avatar tile>
            <img v-if="item.type === 'tab'" :src="loadExtFile('assets/tab.svg')" >
            <img v-else-if="item.type === 'bookmark'" :src="loadExtFile('assets/bookmark.svg')" >
            <img v-else-if="item.type === 'closedTab'" :src="loadExtFile('assets/lamp.svg')" >
            <img v-else-if="item.type === 'keyword'" :src="loadExtFile('assets/keyword.svg')" >
          </v-list-tile-avatar>
...
export default Vue.extend({
  ...
  methods: {
    ...
    controlSelectedItem(direction: 'up' | 'down') {
      ...
      this.$nextTick(() => {
        (this.$refs.scrollTarget as HTMLElement).scrollTop = 72 * this.selectedIndex
      })
    }
  }
})
